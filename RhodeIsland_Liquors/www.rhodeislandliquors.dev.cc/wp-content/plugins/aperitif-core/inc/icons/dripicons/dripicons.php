...
if ( ! function_exists( 'aperitif_core_add_dripicons_to_collection' ) ) {
	function aperitif_core_add_dripicon_to_collection( $icons ) {
		$icons[] = 'AperitifCoreDripiconsIconPack';
		
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'aperitif_core_add_dripicon_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class AperitifCoreDripiconsIconPack extends QodeFrameworkIconPack {
		...
		public function add_icon_pack() {
			$this->set_base( 'dripicons' );
			$this->set_name( 'Dripicons' );
