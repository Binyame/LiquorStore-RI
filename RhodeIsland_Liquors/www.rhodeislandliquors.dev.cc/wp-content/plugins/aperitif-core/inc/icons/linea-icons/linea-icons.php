...
if ( ! function_exists( 'aperitif_core_add_linea_icons_to_collection' ) ) {
	function aperitif_core_add_linea_icons_to_collection( $icons ) {
		$icons[] = 'AperitifCoreLineaIconsPack';
		
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'aperitif_core_add_linea_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class AperitifCoreLineaIconsPack extends QodeFrameworkIconPack {
		...
		public function add_icon_pack() {
			$this->set_base( 'linea-icons' );
			$this->set_name( 'Linea Icons' );
		...
		function specific_icons() {
			return array(
				'search'        => 'icon-basic-magnifier',
				'dropdown-cart' => 'icon-ecommerce-basket',
				'back-to-top'   => 'icon-arrows-up',
				'close'         => 'icon-arrows-remove',
			);
		}
	}
}